const SUPABASE_URL = 'https://ybccbyyrrxdzarsgylql.supabase.co';
        ...
        try { localStorage.removeItem('smachimSession'); } catch(_) {}
        ...
        function clearAllPersonalBrowserStorage() {
            try {
                ...
                [...Object.keys(sessionStorage)].forEach(key => {
                    if (key.startsWith(EVENT_DRAFT_PREFIX) || key.startsWith('smachimEventDraft')) sessionStorage.removeItem(key);
                });
        ...
        function clearRuntimeUserData() {
            ...
            if($('admin-area-root')) $('admin-area-root').innerHTML='';
        ...
        function splitDeclarativeParts(text, separator) {
            ...
            for (const ch of String(text||'')) {
                ...
                if (ch===separator) { if(current.trim()) out.push(current.trim()); current=''; }
                else current+=ch;
            }
        ...
        function updateHeaderActions(){const box=$('header-actions');if(!box)return;if(sessionToken&&currentProfile&&currentIsAdmin&&adminMode){box.innerHTML='<div class="flex gap-4 items-center"><button data-onclick="navigate(\'home\')" class="header-home-action text-sm font-semibold text-slate-500 hover:text-brand-600"><i class="fa-solid fa-house ml-1"></i> דף הבית</button><button data-onclick="loadAdminArea()" class="text-sm font-bold text-brand-700 hover:text-brand-900"><i class="fa-solid fa-shield-halved ml-1"></i> ניהול</button></div>';enhanceAccessibility(box);return;}box.innerHTML=sessionToken&&currentProfile?'<div class="flex gap-4 items-center"><button data-onclick="navigate(\'home\')" class="header-home-action text-sm font-semibold text-slate-500 hover:text-brand-600"><i class="fa-solid fa-house ml-1"></i> דף הבית</button><button data-onclick="loadDashboard()" class="text-sm font-bold text-brand-700 hover:text-brand-900"><i class="fa-solid fa-user ml-1"></i> אזור אישי</button></div>':'<button data-onclick="navigate(\'home\')" class="header-home-action text-sm font-semibold text-slate-500 hover:text-brand-600 transition"><i class="fa-solid fa-house ml-1"></i> דף הבית</button>';enhanceAccessibility(box);}
        ...
        function normalizedDesign(raw={}){
            ...
            return {
                ...
                logoCropDesktop:Math.max(72,Math.round(boundedDesignNumber(raw.logo_width_desktop,380,220,700)*0.29)),
                logoCropMobile:Math.max(46,Math.round(boundedDesignNumber(raw.logo_width_mobile,220,140,320)*0.29)),
                headerDesktop:0,
                headerMobile:0,
        ...
        function applySiteDesign(designOverride=null){
            ...
            root.style.setProperty('--site-logo-crop-height-desktop',d.logoCropDesktop+'px');
            root.style.setProperty('--site-logo-crop-height-mobile',d.logoCropMobile+'px');
        ...
        async function loadDashboard(){
            ...
            if(mode==='volunteer'){
                ...
                if(currentProfile.role==='volunteer'){
                    setDashHtml('dash-cta','<div class="glass-card p-6 flex flex-wrap gap-3 items-center justify-between"><div><h3 class="font-bold text-lg">יש לך גם אירוע?</h3><p class="text-sm text-slate-500">אותו חשבון יכול לשמש גם כבעל שמחה.</p></div><button data-onclick="openAddEvent()" class="btn-dark px-4 py-2">פרסם אירוע</button></div>');
                }
            }else{
                tasks.push(renderHostEvents().then(x=>setDashHtml('dash-main-a',x)));
                if(currentProfile.role==='host'){
                    setDashHtml('dash-cta','<div class="glass-card p-6 flex flex-wrap gap-3 items-center justify-between"><div><h3 class="font-bold text-lg">רוצה גם לשמח?</h3><p class="text-sm text-slate-500">השלם שנת לידה, כתובת והעדפות ונפתח לך אזור משמח.</p></div><button data-onclick="openProfileSettings()" class="btn-brand px-4 py-2">הגדר פרטי משמח</button></div>');
                }
            }
        ...
        async function openFamilyChildArea(childId){
            ...
            box.innerHTML=
                '<div class="glass-card p-6"><div class="flex flex-wrap justify-between gap-3"><div><h3 class="text-xl font-bold">ההתנדבויות של '+esc(child.full_name)+'</h3><p class="text-sm text-slate-500">כל הפעולות מנוהלות מהחשבון שלך וה-SMS נשלח לטלפון שלך.</p></div><button data-onclick="deleteFamilyChild(\''+esc(child.id)+'\')" class="bg-red-50 text-red-700 border border-red-200 rounded-lg px-3 py-2 text-sm font-bold">הסר מהחשבון</button></div><div class="space-y-3 mt-4">'+
                (activeRegs.length?activeRegs.map(r=>'<div class="border rounded-xl p-4 bg-slate-50"><div class="flex flex-wrap justify-between gap-3"><div><b>'+esc(r.event_name)+'</b><div class="text-sm text-slate-500">'+esc(eventTypeLabel(r.event_type))+' · '+esc(formatDate(r.event_date))+' · '+esc(timeRange(r.start_time,r.close_time))+'</div><div class="text-sm text-brand-700 mt-1">📍 '+esc((r.hall_name?r.hall_name+' · ':'')+(r.hall_address||fullAddress(r.city,r.street,r.house_number)))+'</div></div><div class="flex flex-wrap gap-2"><button data-onclick="openEventChat(\''+esc(r.event_id)+'\')" class="btn-soft px-3 py-2 text-xs">צ׳אט</button>'+(!r.attendance_confirmed_at?'<button data-onclick="confirmFamilyChildAttendance(\''+esc(child.id)+'\',\''+esc(r.event_id)+'\')" class="btn-brand px-3 py-2 text-xs">אישור הגעה</button>':'')+'<button data-onclick="cancelFamilyChildRegistration(\''+esc(child.id)+'\',\''+esc(r.event_id)+'\')" class="bg-red-50 text-red-700 rounded-lg px-3 py-2 text-xs font-bold">ביטול</button></div></div></div>').join(''):'<p class="text-slate-500">אין כרגע הרשמות פעילות.</p>')+
                '</div></div>'+
                ...
                ((matches||[]).length?(matches||[]).map(e=>'<div class="border rounded-xl p-4 bg-slate-50"><div class="flex flex-wrap justify-between gap-3"><div><b>'+esc(e.event_name)+'</b><div class="text-sm text-slate-500">'+esc(eventTypeLabel(e.event_type))+' · '+esc(formatDate(e.event_date))+' · '+esc(timeRange(e.start_time,e.close_time))+' · '+esc(e.distance_km)+' ק״מ</div><div class="text-sm text-brand-700 mt-1">📍 '+esc((e.hall_name?e.hall_name+' · ':'')+e.hall_address)+'</div>'+(e.exact_lat==null?'<div class="text-xs text-slate-500 mt-1">הכתובת המדויקת תוצג לאחר ההרשמה.</div>':'')+'</div><button data-onclick="registerFamilyChildForEvent(\''+esc(child.id)+'\',\''+esc(e.id)+'\')" class="btn-brand px-4 py-2 text-sm">רשום/י לאירוע</button></div></div>').join(''):'<p class="text-slate-500">אין כרגע אירועים מתאימים.</p>')+
                '</div></div>';
        }
            ...
            box.innerHTML=data?.length?'<div class="border-t pt-3 space-y-3">'+data.map(r=>{
                ...
                const action=ended&&r.status==='approved'
                  ? (r.host_attendance_confirmed_at
                    ? '<button data-onclick="setHostAttendance(\''+esc(eventId)+'\',\''+esc(r.user_id)+'\',false)" class="btn-soft px-3 py-2 text-xs">בטל אישור בפועל</button>'
                    : '<button data-onclick="setHostAttendance(\''+esc(eventId)+'\',\''+esc(r.user_id)+'\',true)" class="btn-brand px-3 py-2 text-xs">אשר הגעה בפועל</button>')
                  : '';
        ...
        function renderAdminPanel(){
            ...
            return '<div class="space-y-5">'+
                '<div class="glass-card p-3"><div class="flex flex-wrap gap-2">'+tabs.map(t=>
                    '<button id="admin-tab-'+t[0]+'" data-onclick="adminOpenTab(\''+t[0]+'\')" class="btn-soft px-3 py-2 text-sm"><i class="fa-solid '+t[2]+' ml-1"></i>'+t[1]+'</button>'
                ).join('')+'</div></div>'+
        ...
        async function adminRenderOverview(){
            ...
            return '<div class="space-y-5">'+
                '<div class="glass-card p-6"><div class="flex flex-wrap items-center justify-between gap-3"><div><h3 class="text-2xl font-bold">תמונת מצב</h3><p class="text-sm text-slate-500">הנתונים המרכזיים של האתר בזמן אמת.</p></div><button data-onclick="adminOpenTab(\'overview\')" class="btn-soft px-3 py-2"><i class="fa-solid fa-rotate ml-1"></i>רענן</button></div>'+
        ...
        async function adminRenderUsers(){
            ...
            return adminNewUserForm()+'<div class="glass-card p-6"><div class="flex flex-wrap justify-between gap-3 items-end"><div><h3 class="text-2xl font-bold">משתמשים</h3><p class="text-sm text-slate-500">חיפוש, הוספה, מחיקה, בדיקת פרופיל, חסימה, SMS וניתוק סשנים.</p></div><div class="flex flex-wrap gap-2"><button data-onclick="adminToggleNewUserForm(true)" class="btn-brand px-4 py-2"><i class="fa-solid fa-user-plus ml-1"></i> הוסף משתמש</button><input id="admin-user-search" class="input-clean min-w-56" value="'+esc(adminUserSearch)+'" placeholder="שם, טלפון או עיר"><button data-onclick="adminSearchUsers()" class="btn-dark px-4">חפש</button></div></div>'+
                '<div class="mt-5 space-y-3">'+(adminUsersCache.length?adminUsersCache.map(u=>
                    ...
                    '<div class="flex flex-wrap gap-2 items-start"><button data-onclick="adminShowUser(\''+esc(u.id)+'\')" class="btn-soft px-3 py-2">פרטים</button><button data-onclick="adminToggleBlock(\''+esc(u.id)+'\','+(!u.is_blocked)+')" class="'+(u.is_blocked?'bg-green-50 text-green-700':'bg-red-50 text-red-700')+' border rounded-lg px-3 py-2 font-bold text-sm">'+(u.is_blocked?'פתח חסימה':'חסום')+'</button><button data-onclick="adminSetUserSms(\''+esc(u.id)+'\','+(!u.sms_notifications_enabled)+')" class="btn-soft px-3 py-2">'+(u.sms_notifications_enabled?'כבה SMS':'הפעל SMS')+'</button><button data-onclick="adminForceLogout(\''+esc(u.id)+'\')" class="btn-soft px-3 py-2">נתק</button>'+(!u.is_admin?'<button data-onclick="adminDeleteUser(\''+esc(u.id)+'\',\''+esc(u.full_name)+'\')" class="bg-red-50 text-red-700 border border-red-200 rounded-lg px-3 py-2 font-bold text-sm">מחק</button>':'')+'</div></div>'+
        ...
        async function adminShowUser(id){
            ...
            detail.innerHTML='<div class="glass-card p-6 mt-5 border-2 border-slate-300"><div class="flex justify-between gap-3"><div><h3 class="text-xl font-bold">'+esc(p.full_name||'משתמש')+'</h3><p class="text-sm text-slate-500" dir="ltr">'+esc(displayPhone(p.phone||''))+'</p></div><button data-onclick="adminCloseDetail()" class="btn-soft px-3">סגור</button></div>'+
        ...
        async function adminShowEvent(id){
            ...
            const detail=$('admin-detail-panel');if(!detail)return;
            detail.innerHTML='<div class="glass-card p-6 mt-5 border-2 border-slate-300"><div class="flex justify-between gap-3"><div><h3 class="text-xl font-bold">'+esc(e.event_name||'אירוע')+'</h3><p class="text-sm text-slate-500">'+esc(eventTypeLabel(e.event_type))+' · '+esc(formatDate(e.event_date))+' · '+esc(String(e.start_time||'').slice(0,5))+'</p></div><button data-onclick="adminCloseDetail()" class="btn-soft px-3">סגור</button></div>'+
        ...
        async function adminRenderSettings(){
            ...
            return '<div class="space-y-5">'+
                ...
                '<div class="glass-card p-6"><div class="flex flex-wrap justify-between gap-3"><div><h3 class="text-2xl font-bold">לוגו ותמונת דף הבית</h3><p class="text-sm text-slate-500">הקבצים עולים לאחסון מאובטח דרך השרת. מפתח השירות של Supabase לא נשלח לדפדפן.</p></div><button data-onclick="adminSaveContentEditor()" class="btn-dark px-5 py-3">שמור תוכן ועיצוב</button></div>'+
                ...
                '<div class="glass-card p-6"><div class="flex flex-wrap justify-between gap-3 items-center"><div><h3 class="text-2xl font-bold">גדלים ועיצוב</h3><p class="text-sm text-slate-500">שליטה בטוחה בערכי תצוגה מוגבלים. אין כאן CSS חופשי ולכן אי אפשר להזריק קוד.</p></div><button data-onclick="adminSaveContentEditor()" class="btn-dark px-5 py-3">שמור עיצוב</button></div>'+
                    '<div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 mt-5">'+
                        ...
                        '<label><span class="field-label">גובה תמונת הבית במחשב (px, 0 = טבעי)</span><input id="admin-design-hero-desktop" type="number" min="0" max="1000" class="input-clean" value="'+Number(design.hero_height_desktop||0)+'"><span class="field-help">ערך מעל 0 מגדיל את הבאנר גם כלפי מטה וממלא את כל הגובה.</span></label>'+
                        '<label><span class="field-label">גובה תמונת הבית בטלפון (px, 0 = טבעי)</span><input id="admin-design-hero-mobile" type="number" min="0" max="650" class="input-clean" value="'+Number(design.hero_height_mobile||0)+'"><span class="field-help">ערך מעל 0 מגדיל את שטח התמונה לגובה.</span></label>'+
                    ...
                    '</div>'+
                    '<p class="text-xs text-slate-400 mt-4">שוליים 0 מצמידים את תוכן הלוגו כמעט לשולי הפס — כולל חיתוך השוליים הלבנים המובנים בתמונת הלוגו. גובה תמונת הבית 0 מציג את היחס הטבעי; כל ערך גדול מ־0 מגדיל את הבאנר בפועל כלפי מטה וממלא את השטח, ולכן ייתכן חיתוך קל בצדדים.</p>'+
                '</div>'+
                ...
                '</div><button data-onclick="adminOpenTab(\'sms\')" class="btn-soft px-4 py-2 mt-4">פתח הגדרות SMS מלאות</button></div>'+
            '</div>';
        }
        ...
        async function adminSaveContentEditor(){
            ...
            const heroHeightDesktop=designInt('admin-design-hero-desktop',base.design?.hero_height_desktop??0,0,1000);
            const heroHeightMobile=designInt('admin-design-hero-mobile',base.design?.hero_height_mobile??0,0,650);
            const design={
                ...
                hero_fit:(heroHeightDesktop||heroHeightMobile)?'cover':($('admin-design-hero-fit')?.value==='cover'?'cover':'contain'),
